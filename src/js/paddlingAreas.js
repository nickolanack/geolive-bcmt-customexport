...
function PaddlingRegionMapSearchBehavior(regionsData, map, kmlUrl) {
    ...
    var detail = new Element('div', {
        'class': 'paddling-areas-detail',
        html: '<span class="no-region">select a region</span>'
    });
    var clear = new Element('button', {
    ...
    var PaddlingAreasSelection = new Class({
        ...
        initialize: function(regionsData, map, kmlUrl) {
            ...
            (new XMLControlQuery(kmlUrl, "",
                ...
                (new KmlReader(xml)).parsePolygons(function(polygonParams) {
        ...
        _setAreaPoverTextSelectable(area) {
            ...
            popover.setText(new Element('span', {
                html: 'click to add <span class="pop-area">' + area + '</span> to the current selection',
                style: "width:200px;"
            }));
            return popover;
        },
        _setAreaPoverTextRemoveable(area) {
            var popover = popoverByArea(area);
            popover.setText(new Element('span', {
                html: 'click to remove <span class="pop-area remove">' + area + '</span> from the current selection',
                style: "width:200px;"
            }));
            return popover;
        },
        ...
        _overRegionWithArea: function(area) {
            ...
            var popover = popoverByArea(area);
            popover.setText(new Element('span', {
                html: 'click to choose <span class="pop-area region">' + region + '</span> as the current region',
                style: "width:200px;"
            }));
            popover.show();
    ...
    window.Outlets = {
        ...
        onChange: function(fn) {
            ...
            mapselector.addEvent('selectRegion', function() {
                fn(mapselector);
            });
            mapselector.addEvent('clearRegion', function() {
                fn(mapselector);
            });

            mapselector.addEvent('selectArea', function() {
                fn(mapselector);
            });
            mapselector.addEvent('clearArea', function() {
                fn(mapselector);
            });
            mapselector.addEvent('reset', function() {
                fn(mapselector);
            });
